...
def print_to_file_in_aggregation(text, file_name):
...
def simple_median(w):
    print("type of gradient list: ")
    print(type(w), type(w[0]))
    device = w[0][list(w[0].keys())[0]].device
    w_med = copy.deepcopy(w[0])
    for k in w_med.keys():
        shape = w_med[k].shape
    ...
    print("type of returned dictionaary", type(w_med))
    return w_med
...
def average_weights(w, marks):
    # assert abs(sum(marks) - 1.0) < 0.02, "Marks must sum to 1"
    assert len(w) == len(marks), "Number of models and marks must match"
    print_to_file_in_aggregation(f"{sum(marks)}", "/home/ndag/Abid/AntiFlipper/stats/marks.txt")
    print_to_file_in_aggregation(f"{marks}", "/home/ndag/Abid/AntiFlipper/stats/marks.txt")
...
def average_weights(w, marks):
    ...
    for key in w_avg.keys():
        original_dtype = w_avg[key].dtype
        w_avg[key] = w_avg[key] * marks[0]
        
    for key in w_avg.keys():
        for i in range(1, len(w)):
            # Convert the weighted update to the same dtype before adding
            weighted_update = (w[i][key] * marks[i]).to(original_dtype)
            w_avg[key] += weighted_update
            
        w_avg[key] = (w_avg[key] * (1/sum(marks))).to(original_dtype)
    return w_avg
